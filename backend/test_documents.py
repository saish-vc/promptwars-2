...
def main() -> None:
    ...
    try:
        ...
        assert extract_text("contract.pdf", _pdf_bytes()) == "PDF clause"

        doc_id2 = store.save("typed clause", "typed.txt")
        assert store.get_text(doc_id2) == "typed clause"
        ...
        while time.time() < deadline and os.path.exists(db_path):
            try:
                os.unlink(db_path)
                break
